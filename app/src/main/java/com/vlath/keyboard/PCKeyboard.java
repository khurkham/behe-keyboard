...
/**
 * Main class of the keyboard, extending InputMethodService. Here we handle all
 * the user interaction with the keyboard itself.
 */
...
public class PCKeyboard extends InputMethodService
    ...
    /**
     * As we don't want to bother changing the app theme, we use filters to theme
     * the keyboard. Each filter needs an array of colors. The arrays are declared
     * below.
     */

    // TODO: Add the arrays in a separate, static class, so they are eay to access
    // and modify
     ...
     * generated. This will be called the first time your input method is displayed,
     * and every time it needs to be re-created such as due to a configuration
     * change.
     *
     * We also s
     */
    ...
    /**
     * This is the main point where we do our initialization of the input method to
     * begin operating on an application. At this point we have been bound to the
     * client, and are now receiving all of the detailed information about the
     * target of our edits.
     *
     *
     * And we have to reinitialize all we've one to make sure the keyboard aspect
     * matches The one selected in settings.
     */
    @Override
    public void onStartInput(EditorInfo attribute, boolean restarting) {
        ...
        updateCandidates();
        /**
         * Some code on here is based on the SoftKeyboard Sample. I don't fully
         * understand it. I need to look it up and delete any unnecessary stuff.
         */
        if (!restarting) {
            // Clear shift states.
            mMetaState = 0;
    ...
    /**
     * Helper to update the shift state of our keyboard based on the initial editor
     * state.
     */
    ...
    /**
     * Update the list of available candidates from the current composing text. This
     * will need to be filled in by however you are determining candidates.
     */
    ...
    /**
     * http://www.tutorialspoint.com/android/android_spelling_checker.htm Sort of
     * copy-paste, huh.
     *
    ...
    private void dumpSuggestionsInfoInternal(final List<String> sb, final SuggestionsInfo si, final int length,
            final int offset) {
        // Returned suggestions are contained in SuggestionsInfo
        ...
        try {
            ...
            for (final SentenceSuggestionsInfo ssi : results) {
                for (int j = 0; j < ssi.getSuggestionsCount(); ++j) {
                    dumpSuggestionsInfoInternal(sb, ssi.getSuggestionsInfoAt(j), ssi.getOffsetAt(j),
                            ssi.getLengthAt(j));
                }
            }
    ...
    private void processKeyCombo(int keycode) {
        ...
        if (Variables.isAnyOn()) {
            if (Variables.isCtrl() && Variables.isAlt()) {
                getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                        getHardKeyCode(keycode), 0, KeyEvent.META_CTRL_ON | KeyEvent.META_ALT_ON));
                getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_UP,
                        getHardKeyCode(keycode), 0, KeyEvent.META_CTRL_ON | KeyEvent.META_ALT_ON));
            } else {
                if (Variables.isCtrl()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                            getHardKeyCode(keycode), 0, KeyEvent.META_CTRL_ON));
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_UP,
                            getHardKeyCode(keycode), 0, KeyEvent.META_CTRL_ON));
                }
                if (Variables.isAlt()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                            getHardKeyCode(keycode), 0, KeyEvent.META_ALT_ON));
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_UP,
                            getHardKeyCode(keycode), 0, KeyEvent.META_ALT_ON));
                }
            }
    ...
    private void setInputType() {
        ...
        /**
         * Checks the preferences for the default keyboard layout. If standard, we start
         * out whether in standard or numbers, depending on the input type.
         */
    ...
    private void capsOnFirst() {
         ...
         * Accordingly to the official android documentation, if the caps mode is not
         * equal to 0, We should start in caps mode. Although, tests have proven that
         * additionally checks are needed. I'll see what I can do on this.
         */
    ...
    public void onKey(int primaryCode, int[] keyCodes) {
        ...
        case Keyboard.KEYCODE_SHIFT:
            ...
            /**
             * We need to check whether we are on symbols layout or not. Then, perform the
             * operation accordingly. Also, we check for double tab on the shift, and, if
             * detected We set a global variable that tells us that the Shift is in the lock
             * position.
             */
        ...
        case LatinKeyboard.KEYCODE_LAYUOUT_SWITCH:
            ...
            /**
             * Language Switch is a custom value defined in the LatinKeyboard class. We use
             * it to switch between standard/arrow keys/programming layouts.
             */
        ...
        case LatinKeyboard.KEYCODE_ESCAPE:
            ...
            getCurrentInputConnection()
                    .sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_ESCAPE, 0));
            getCurrentInputConnection()
                    .sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_UP, KeyEvent.KEYCODE_ESCAPE, 0));
            break;
        ...
        case LatinKeyboard.KEYCODE_STANDARD_SWITCH:
            ...
            /**
             * This key enables the user to switch rapidly between standard/arrow keys
             * layouts.
             */
        ...
        case LatinKeyboard.KEYCODE_DELL_PROCESS:
            ...
            if (Variables.isAnyOn()) {
                if (Variables.isCtrl() && Variables.isAlt()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                            KeyEvent.KEYCODE_DEL, 0, KeyEvent.META_CTRL_ON | KeyEvent.META_ALT_ON));
                }
                if (Variables.isAlt()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                ...
                getCurrentInputConnection()
                        .sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DEL, 0));
                getCurrentInputConnection()
                        .sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_UP, KeyEvent.KEYCODE_DEL, 0));
            }
            break;
        case LatinKeyboard.KEYCODE_I_DONT_KNOW_WHY_I_PUT_THAT_HERE:
            ...
            if (Variables.isAnyOn()) {
                if (Variables.isCtrl() && Variables.isAlt()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                            KeyEvent.KEYCODE_TAB, 0, KeyEvent.META_CTRL_ON | KeyEvent.META_ALT_ON));
                }
                if (Variables.isAlt()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                            KeyEvent.KEYCODE_TAB, 0, KeyEvent.META_ALT_ON));
                }
                if (Variables.isCtrl()) {
                    getCurrentInputConnection().sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN,
                ...
                getCurrentInputConnection()
                        .sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_TAB, 0));
                getCurrentInputConnection()
                        .sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_UP, KeyEvent.KEYCODE_TAB, 0));
            }
    ...
    public void setStandardKeyboard() {
        ...
        int layout = Integer
                .parseInt(PreferenceManager.getDefaultSharedPreferences(getBaseContext()).getString("layout", "1"));
